...
def create_children_with_exec(n, command):
    ...
    for i in range(n):
        pid = os.fork()
        if pid == 0:
            print(f"[Child {i+1}] PID={os.getpid()}, Parent PID={os.getppid()}, executing command: {' '.join(command)}\n")
            try:
...
def main():
    ...
    print(f"[Parent] PID={os.getpid()}, creating {n} children to run {' '.join(command)}")
